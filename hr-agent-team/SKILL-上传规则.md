...
## 0. 主库与提交方式（Pull Request）

- **主库（GitHub）**：`https://github.com/maoshiking/hr-agent-team-up.git`（默认分支 `main`）
- **提交方式（协作标准）**：
  1. 每个成员用**自己命名的一个分支**（如 `member-a`、`scout`，名字自己定并报给团队）；
  2. 只在**自己的分支**上改，且仍只改自己那一个 `.java`（纪律同下）；
  3. 完成/更新后，把分支推送到主库，并**发 Pull Request** 合并到 `main`（由整合者 review）；
  4. 除非整合者明确说"本次直接推 main"，否则**不直接推 main**。
- **阶段说明**：搭建初期/整合者整体同步文件时可临时直接推 `main`（已做过）；**进入各自开发后一律走自己的分支 + PR**。
