...
> **给一句模糊的招聘诉求，5 个数字员工自主接力，端到端完成"需求澄清 → JD → 简历筛选 → 线上笔试 → 题库化面试 → 测评背调 → 综合评分 → offer 与入职计划"的完整闭环，交付 16 类可验收成果（报告/表格/网页，均为真实文件），全程可见工具调用与人工确认节点。**
...
| 全程 | 一条模糊指令 | 16 类成果至少产出 3 类，含报告/表格/网页（赛题要求） |
